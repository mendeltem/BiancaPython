...
bianca_results = [i for i in get_all_dirs(datasets_prepared_path) if "result" in i][0]
...
for bianca_result_path in [bianca_results]:
    ...
    bianca_result_path_basename = os.path.basename(bianca_result_path)

    nulled_images_paths_list    = get_all_dirs(nulled_images_path)
    bianca_result_paths_list    = get_all_dirs(bianca_result_path)
    
    
    result_data_path           = os.path.join(bianca_result_path,"result_data.xlsx")
    result_paths_path           = os.path.join(bianca_result_path,"result_paths.xlsx")
    ...
    for bri, bianca_result in enumerate(bianca_result_paths_list):
        ...
        new_maskpath = join(bianca_result,os.path.basename(mask_image))
        os.system(f"cp {mask_image} {new_maskpath}")
        
        
        new_flair_image = join(bianca_result,os.path.basename(flair_image))
        os.system(f"cp {flair_image} {new_flair_image}")
        ...
        for threshold in thresholds_list:
            ...
            for minimum_clustersize in minimum_clustersizes:
                ...
                for distance_to_ventrickle in distance_to_ventrickles:
                    ...
                    df_result_path_temp["mask_path"]     = new_maskpath
                    df_result_path_temp["flair_path"]     = new_flair_image
