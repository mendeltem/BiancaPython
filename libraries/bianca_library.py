...
def create_training_master_file(train_subjects, data_set_df,bianca_master_file_directory, master_file_name, mni_bool = False, normal_bool=True):
    ...
    masterfiles_path =os.path.join(bianca_master_file_directory, "masterfiles")
    masterfiles_df_path =os.path.join(bianca_master_file_directory, "masterfiles_df")
    create_dir(masterfiles_path)
    create_dir(masterfiles_df_path)
    ...
    for train_index,subject in enumerate(train_subjects):
        ...
        if normal_bool:
            ...
            flair_path                =  patient_df['flair_normalized_path'].values[0]
            mask_path                =   patient_df['mask_normalized_path'].values[0]
            
        else:
            flair_path                =  patient_df['flair_path'].values[0]
            mask_path                =   patient_df['mask_path'].values[0]
        ...
        if flair_path or mask_path:
            ...
            if mni_bool:
                ...
                master_file_train=    flair_path+"  "+mask_path
    ...
    master_file_path = join(masterfiles_path,f"{master_file_name}_train_master_file.txt") 
    master_file_df_path = join(masterfiles_df_path,f"{master_file_name}_train_master_file.xlsx") 
...
class MasterCreator:
    ...
    def __init__(self, bianca_master_files_directory, data_set_df):
        ...
        create_dir(bianca_master_files_directory)
        
        self.train_subjects_directory         =     os.path.join(bianca_master_files_directory,"train_subjects")
        create_dir(self.train_subjects_directory)
        
        self.test_subjects_directory         =     os.path.join(bianca_master_files_directory,"test_subjects")
        create_dir(self.test_subjects_directory)


    # Method to accelerate the car's speed
    def create_master_directory(self, train_subjects,master_file_name,mni_bool = False,normal_bool=True):
        create_training_master_file(train_subjects, self.data_set_df,self.train_subjects_directory,master_file_name,mni_bool, normal_bool)
        
        
        
    # Method to accelerate the car's speed
    def create_test_subject_df(self, test_subjects,master_file_name):
        
        
        test_dir                             =     os.path.join(self.bianca_master_files_directory,"test_subjects")
        bianca_master_file_directory         =     os.path.join(test_dir,master_file_name)
    ...
    # Method to accelerate the car's speed
    def create_train_subject_df(self, train_subjects,master_file_name):
        bianca_master_file_directory         =     os.path.join(self.bianca_master_files_directory,master_file_name)
        

        
        #create_dir(train_subjects_directory)
        #create_test_subjects_for_bianca_table(test_subjects,self.data_set_df,bianca_master_file_directory)
        create_train_subjects_for_bianca_table(train_subjects,self.data_set_df,self.train_subjects_directory)
        
        
        
        
def create_masterfile_for_test(master_file_path,
                                biascorrected_bet_image_path,
                                mask_path,
                                MNI_xfm_path,
                                mni_bool
    ...
    temp_master_file = join(dirname,"temp_master_file")
    
    create_dir(temp_master_file)
    

    with open(master_file_path, 'r') as file:
        master_file_txt = file.read()
        
    row_number =master_file_txt.count("WMHmask")+1
    ...
    if mni_bool:
        ...
        master_file_test = biascorrected_bet_image_path+" "+mask_path


    all_master_file_txt  = master_file_txt + "\n"+master_file_test

    new_master_file_path = join(temp_master_file,"master_file.txt")
...
def run_bianca(master_file_path,
    ...
    return output_mask_path,train_bianca_commands
