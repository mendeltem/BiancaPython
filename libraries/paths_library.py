...
from path_utils import get_all_dirs
    ...
    for hlm_i,hlm_path in enumerate(hlm_list):
        ...
        for f,files in enumerate(list_subjects):
            ...
            for file_path in list_files: 
                ...
                # if 'wmh.' in basename(file_path).lower():
                #     mask_type = "challange"
            ...
            #patient_df["mask_type"] = mask_type   
            ...
            mask_volume = round(get_volume( mask_image) ,2)

            print(f"mask_volume {mask_volume}")
            patient_df["lesion_concentration_voxel_volume"] = mask_volume
...
def create_belove_table(dataset_path,index_for_patient):
    ...
    # file_base_names = {}
    # nifi_list = [os.path.join(dataset_path,i) for i in os.listdir(dataset_path)  if ".nii" in i]
    
    
    # for f,file in enumerate(nifi_list):
    ...
    #         mask_volume = round(get_volume( os.path.join(dataset_path,found_roi)) ,2)
    ...
    for hlm_i,hlm_path in enumerate(hlm_list):
        ...
        for f_key, f_value in file_base_names.items():
            ...
            mask_volume = round(get_volume( join(dname,f_value['mask'])) ,2)
